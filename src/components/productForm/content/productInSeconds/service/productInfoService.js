...
const useProductService = () => {
  ...
  const processProductInfo = async (
    selectedSource,
    inputValue,
    setLoadingState
  ) => {
    ...
    try {
      ...
      if (selectedSource === "dropi") {
        ...
        const productData = await autoCreateFormService.getProductById(
          productId
        );
        productObject = productData?.objects;
      } else if (selectedSource === "url") {
        console.log("Obteniendo producto desde URL:", productId);
        const response = await autoCreateFormService.getProductsByUrl(
          productId
        );
        ...
        const productData = await autoCreateFormService.getShopifyProductById(
          productId
        );
      ...
      const generatedJson = await autoCreateFormService.generateJson(
        productObject
      );
...
const mapApiDataToContext = (
  ...
  const reminder2Data = parseTimeAndUnit(
    recordatoriosData.tiempo_recordatorio_2
  );
  ...
  return {
    ...
    voice: {
      ...
      useSpeakerBoost: String(apiData.voz_con_ia?.speaker_boost) === "true",
      useVoiceAI: apiData.voz_con_ia?.habilitar === "si",
    },
    reminder: {
      reminder1: {
