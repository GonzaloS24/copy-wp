...
const autoCreateFormService = {
  ...
  async getShopifyProductById(productId) {
    try {
      ...
      const response = await axios.get(
        `${BACK_BASE_URL}/api/integrations/shopify/products/${productId}?shop=${encodeURIComponent(shopifyCredentials.url)}&access_token=${encodeURIComponent(shopifyCredentials.token)}`,
        {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`
